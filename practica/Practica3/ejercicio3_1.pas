...
procedure borrarTruncando(var arc_empleado: fEmpleado);
	var
		encontro:boolean;
		r:rEmpleado;
		numBorrar,posBorrar:integer;
	begin
		reset(arc_empleado);
		writeln('Ingrese num de empleado a borrar');
		readln(numBorrar);
		encontro:=false;
		while (not eof(arc_empleado)) and (not encontro) do begin
			read(arc_empleado,r);
			if (r.num= numBorrar) then encontro:=true;
		end;
	...
	{copiando el último registro del archivo en
la posición del registro a borrar y luego truncando el archivo en la posición del último
registro de forma tal de evitar duplicados. }
	end;
...
begin
	...
	writeln('Ingrese 7 para borrar un empleado por cod de empleado¿?');
	...
	case opcion_elegida of
		...
		'7': borrarTruncando(arc_empleado);
